...
public:
  ...
  LogicalResult matchAndRewrite(dip::Resize2DOp op,
    ...
    if (interpolationAttr ==
      ...
      dip::BilinearInterpolationResizingNew(rewriter, loc, ctx, input, output,
    ...
    // // purpose.
    // Value inputRowLastElem = rewriter.create<arith::SubIOp>(loc, inputRow,
    // c1); Value inputRowLastElemF32 = indexToF32(rewriter, loc,
    // inputRowLastElem);

    // Value inputColLastElem = rewriter.create<arith::SubIOp>(loc, inputCol,
    // c1); Value inputColLastElemF32 = indexToF32(rewriter, loc,
    // inputColLastElem);

    // Value outputRowLastElem =
    //     rewriter.create<arith::SubIOp>(loc, outputRow, c1);
    // Value outputRowLastElemF32 = indexToF32(rewriter, loc,
    // outputRowLastElem);

    // Value outputColLastElem =
    //     rewriter.create<arith::SubIOp>(loc, outputCol, c1);
    // Value outputColLastElemF32 = indexToF32(rewriter, loc,
    // outputColLastElem);
    ...
    //       input, output, horizontalScalingFactorVec,
    //       verticalScalingFactorVec, outputRowLastElemF32,
    //       outputColLastElemF32, inputRowLastElemF32, inputColLastElemF32,
    //       vectorTy32, stride, c0, c0F32);

    //   dip::NearestNeighbourInterpolationResizing(
    //       rewriter, loc, ctx, lowerBounds2, upperBounds2, steps,
    //       strideTailVal, input, output, horizontalScalingFactorVec,
    //       verticalScalingFactorVec, outputRowLastElemF32,
    //       outputColLastElemF32, inputRowLastElemF32, inputColLastElemF32,
    //       vectorTy32, stride, c0, c0F32);
    // } else if (interpolationAttr ==
    ...
    //       verticalScalingFactorVec, outputRowLastElemF32,
    //       outputColLastElemF32, inputRowLastElemF32, inputColLastElemF32,
    //       vectorTy32, stride, c0, c0F32, c1F32);
