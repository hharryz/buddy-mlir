      ...
      loc, notEqual, [&](OpBuilder &tBuilder, Location tLoc) {
        tBuilder.create<scf::ForOp>(
            ...
            [&](OpBuilder &xBuilder, Location xLoc, Value xiv, ValueRange) {
              ...
              // FloatType type =
              //   inElemTy.isF32() ? FloatType::getF32(xBuilder.getContext()) :
              //   FloatType::getF64(xBuilder.getContext());
              // Value halfF = xBuilder.create<arith::ConstantFloatOp>(xLoc,
              // (llvm::APFloat)(float)HALF, type); Value add2 =
              // xBuilder.create<arith::FPToSIOp>(xLoc, xBuilder.getI32Type(),
              // xBuilder.create<arith::AddFOp>(xLoc, add1, halfF));
              //  !! SHIFT = 11
              Value shift = xBuilder.create<arith::ConstantIntOp>(
                  xLoc, 11, xBuilder.getI32Type());
              Value resShifted =
      ...
      [&](OpBuilder &yBuilder, Location yLoc, Value yiv, ValueRange) {
        ...
        yBuilder.create<scf::ForOp>(
            ...
            [&](OpBuilder &xBuilder, Location xLoc, Value xiv, ValueRange) {
              ...
              //  !! HALF = 1 << (SHIFT - 1)
              Value HALF = xBuilder.create<arith::ConstantIntOp>(
                  xLoc, 1 << (11 - 1), xBuilder.getI32Type());
